...
def score(
    ...
    hybrid = (0.2 * tfidf) + (0.5 * sbert) + (0.3 * skill_coverage)
...
def compute_hybrid_score(
    tfidf_score: float, sbert_score: float, skill_coverage: float = 0.0
) -> float:
    return (0.2 * tfidf_score) + (0.5 * sbert_score) + (0.3 * skill_coverage)
